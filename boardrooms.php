...
<div class="wrapper">
  ...
  <div class="container-fluid">
    <div class="row">
      <div class="col-sm-6">
        <h1 class="m-0"><i class="nav-icon fas fa-bed"></i> Boardrooms</h1>
      </div>
    </div>
  </div>
</div>
...
// Dodanie sortowania rosnącego po kolumnie "nr"
$sql .= " ORDER BY nr ASC";
...
if ($result->num_rows > 0) {
  ...
  echo '<thead><tr><th><input type="checkbox" id="selectAll" onchange="selectAllCheckboxes()"></th><th>nr</th><th>pietro</th><th>typ</th><th>miejsca</th><th>Status</th><th>Opcje sali</th></tr></thead>';
  echo '<tbody>';
  echo '<br>';
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (isset($_POST["save_changes"])) {
          ...
          // Walidacja pól pietro i miejsca
          $errorMessages = [];
        
          if ($pietro < 0 || $pietro > 10) {
            $errorMessages[] = 'Pole "Piętro" dla sali ' . $roomNumber . ' musi być wartością między 0 a 10.';
          }
        
          if (empty($miejsca)) {
            $errorMessages[] = 'Pole "Miejsca" dla sali ' . $roomNumber . ' nie może być puste.';
          } elseif ($miejsca < 20 || $miejsca > 150) {
            $errorMessages[] = 'Pole "Miejsca" dla sali ' . $roomNumber . ' musi być wartością między 20 a 150.';
          }
          ...
          if (!empty($errorMessages)) {
            ...
            <script>
              Swal.fire({
                ...
                text: "' . addslashes(implode(' ', $errorMessages)). '",
                showConfirmButton: true
              }).then(function() {
                setTimeout(function() {
                  location.href = location.href; // Przeładowanie strony
                }, 0);
              });
            </script>
            ';
          } else {
            // Sprawdzenie, czy jakiekolwiek dane zostały zmienione
            $select_sql = "SELECT * FROM sale WHERE nr = '$roomNumber'";
            $result = $conn->query($select_sql);
        
            if ($result->num_rows > 0) {
              $row = $result->fetch_assoc();
              if ($row['pietro'] == $pietro && $row['typ'] == $typ && $row['miejsca'] == $miejsca) {
                echo '
                <script>
                  Swal.fire({
                    icon: "info",
                    title: "Brak zmian",
                    text: "Nie dokonano żadnych zmian, ponieważ żadne dane nie zostały zmienione.",
                    showConfirmButton: true
                  }).then(function() {
                    setTimeout(function() {
                      location.href = location.href; // Przeładowanie strony
                ...
                $update_sql = "UPDATE sale SET pietro = '$pietro', typ = '$typ', miejsca = '$miejsca' WHERE nr = '$roomNumber'";
                if ($conn->query($update_sql) === TRUE) {
                  echo '
                  <script>
                    Swal.fire({
                      icon: "success",
                      title: "Dane zmienione pomyślnie",
                      showConfirmButton: true,
                    }).then(function() {
                      setTimeout(function() {
                        location.href = location.href; // Przeładowanie strony
                  ...
                  echo '<div class="alert alert-danger" role="alert">Błąd podczas zapisywania zmian: ' . $conn->error . '</div>';
                }
              }
            }
          }
      ...
      if (isset($_POST["delete_room"]) && !empty($_POST["delete_room"])) {
        $delete_room = $_POST["delete_room"];
    
        // Usuń salę z bazy danych
        $delete_sql = "DELETE FROM sale WHERE nr = '$delete_room'";
        if ($conn->query($delete_sql) === TRUE) {
            echo '<meta http-equiv="refresh" content="0">';
        } else {
            echo '<div class="alert alert-danger" role="alert">Błąd podczas usuwania sali: ' . $conn->error . '</div>';
        }
    }
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        <button type="button" class="btn btn-sm btn-danger" onclick="confirmRoomDeletion(\'' . $row["nr"] . '\')">Usuń sale</button>
        <input type="hidden" name="delete_room" value="' . $row["nr"] . '">
        </form> 
        </div>

        <div style="display: inline-block;">
        <form method="post" action="">
        <button type="button" class="btn btn-sm btn-warning font-weight-bold" onclick="RoomModify(\'' . $row["nr"] . '\')">Modyfikuj sale</button>
        <input type="hidden" name="modify_room" value="' . $row["nr"] . '">
        </form>
        </div>
        </td>';
        echo '</tr>';
...
<script>
function confirmRoomDeletion(roomNumber) {
  var selectedRooms = document.getElementsByName("selected_rooms[]");
    var selectedRoomsArray = [];
    ...
    if (selectedRoomsArray.length > 0) {
        Swal.fire({
            icon: 'error',
            title: 'Błąd',
            text: 'Nie można usunąć sali mając zaznaczone inne sale.'
        });
        return false; // Przerwij zapis lub wysłanie formularza
    }
    ...
    Swal.fire({
        icon: 'question',
        title: 'Potwierdzenie',
        text: 'Czy na pewno chcesz usunąć salę ' + roomNumber + '?',
        showCancelButton: true,
        confirmButtonText: 'Tak',
        cancelButtonText: 'Nie'
    }).then((result) => {
...
<script>
function RoomModify(roomNumber) {
  $.ajax({
    url: 'pobierz_dane_sali.php',
    type: 'POST',
    ...
    success: function(response) {
      ...
      // Wypełnij pola formularza danymi sali
      document.getElementById('pietro').value = data.pietro;
      document.getElementById('typ').value = data.typ;
      document.getElementById('miejsca').value = data.miejsca;
      document.getElementById('roomNumber').value = roomNumber;
      document.getElementById('roomNumberValue').textContent = roomNumber;
      
      // Wyświetl okno modalne
      $('#roomModifyModal').modal('show');
    },
    error: function() {
      alert('Wystąpił błąd podczas pobierania danych sali.');
    }
  });
}
</script>

<div class="modal fade" id="roomModifyModal" tabindex="-1" role="dialog" aria-labelledby="roomModifyModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header bg-primary text-white">
        <h5 class="modal-title" id="roomModifyModalLabel">Modyfikuj salę nr <span id="roomNumberValue"></span></h5>
        <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <form id="roomModifyForm" method="post" action="boardrooms.php">
          <div class="form-group">
            <label for="pietro">Piętro:</label>
            <input type="number" class="form-control" id="pietro" name="pietro" value="<?php echo $pietroValue; ?>">
          </div>
          <div class="form-group">
            ...
            <select class="form-control" id="typ" name="typ" required>
              ...
              <option value="Audytorium">Audytorium</option>
            </select>
          </div>
          <div class="form-group">
            <label for="miejsca">Miejsca:</label>
            <input type="number" class="form-control" id="miejsca" name="miejsca" value="<?php echo $miejscaValue; ?>" required>
          </div>
          <input type="hidden" id="roomNumber" name="roomNumber">
          <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#confirmModal">Zapisz zmiany</button>
        </form>
      </div>
    </div>
  </div>
...
<div class="modal fade" id="confirmModal" tabindex="-1" role="dialog" aria-labelledby="confirmModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header bg-primary text-white">
        <h5 class="modal-title" id="confirmModalLabel">Potwierdź zmiany</h5>
        <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      ...
      <div class="modal-footer">
        ...
        <button type="submit" class="btn btn-primary" name="save_changes" form="roomModifyForm">Zmień</button>
      </div>
    </div>
  </div>
</div>
