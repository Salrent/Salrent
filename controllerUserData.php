...
if (isset($_POST['delete'])) {
    ...
    $run_sql = mysqli_query($con, $check_email);
    if (mysqli_num_rows($run_sql) > 0) {
        $insert_code = "DELETE from user WHERE email = '$email'";
        $run_query = mysqli_query($con, $insert_code);
        if ($run_query) {
            header('Location: login-user.php');
        } else {
            $errors['db-error'] = "Wystąpił błąd z usunięciem konta!";
        }
    }
}
...
if (isset($_POST['aktualizuj'])) {
    ...
    if (!preg_match("/^[a-zA-Z -]+$/", $name)) {
        ...
        $errors['name'] = "imie musi się składać z conajmniej 3 liter!";
    }
    if(!preg_match("/^[a-zA-Z -]+$/", $surname)) {
        $errors['surname'] = "Nazwisko musi się składać tylko z liter!";
    }else if(strlen($surname) < 3) {
        $errors['nazwisko'] = "Nazwisko musi się składać z conajmniej 3 liter!";
    }
    if(!preg_match("/^[a-zA-Z -]+$/", $city)) {
        $errors['city'] = "Miasto musi się składać tylko z liter!";
    }
    if (count($errors) === 0){
        $email = $_SESSION['email'];
        $check_email = "SELECT * FROM user WHERE email='$email'";
        $run_sql = mysqli_query($con, $check_email);
        if (mysqli_num_rows($run_sql) > 0) {
            $insert_code = "UPDATE user SET imie='$name', nazwisko ='$surname', miasto='$city' WHERE email = '$email'";
            $run_query = mysqli_query($con, $insert_code);
            if ($run_query) {
                echo "Dane zostały zaktualizowane";
                header('Location: ustawienia-konta.php');
            } else {
                $errors['db-error'] = "Wystąpił błąd z aktualizacją danych!";
            }
        }
    }
}
...
if (isset($_POST['zmienhaslo'])) {
    ...
    if (empty($_POST['current_password'])) {
        $errors['current_password1'] = "Podaj obecne hasło";
    }
    if(!password_verify($_POST['current_password'], $user['haslo'])) {
        $errors['current_password2'] = "Obecne hasło jest nieprawidłowe";
    }
    if(empty($_POST['new_password']) || empty($_POST['confirm_password'])) {
        $errors['new_password1'] = "Podaj nowe hasło i potwierdź je";
    }
    if($_POST['new_password'] !== $_POST['confirm_password']) {
        $errors['new_password2'] = "Nowe hasło i potwierdzenie hasła nie są identyczne";
    }
    if (!empty($_POST['current_password']) && !empty($_POST['new_password']) && ($_POST['current_password'] === $_POST['new_password'])) {
        $errors['new_password3'] = "Nowe hasło musi być inne niż obecne";
    }
    if (!preg_match('/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[^\w\d\s]).{8,}$/', $_POST['new_password'])) {
        $errors['new_password4'] = "Nowe hasło musi być silne";
    }
    if (count($errors) === 0){
        $email = $_SESSION['email'];
        $encpass = password_hash($_POST['new_password'], PASSWORD_BCRYPT);
        $check_email = "SELECT * FROM user WHERE email='$email'";
        $run_sql = mysqli_query($con, $check_email);
        if (mysqli_num_rows($run_sql) > 0) {
            $insert_code = "UPDATE user SET haslo='$encpass' WHERE email = '$email'";
            $run_query = mysqli_query($con, $insert_code);
            if ($run_query) {
                echo "Dane zostały zaktualizowane";
                header('Location: login-user.php');
